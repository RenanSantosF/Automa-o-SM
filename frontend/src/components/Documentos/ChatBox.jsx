...
const aprovar = async () => {
  if ((doc.status !== 'enviado' && doc.status !== 'reprovado') || userData.setor !== 'ocorrencia') {
...
<div className="flex flex-wrap gap-2 px-4 py-3 border-b bg-white text-sm">
  {userData.id === doc.usuario_id && doc.status === 'reprovado' && (
    <button
      ...
      className="flex items-center gap-1 bg-blue-100 hover:bg-blue-200 text-blue-800 border border-blue-300 px-3 py-1.5 rounded-md shadow-sm transition"
    >
  ...
  {userData.setor === 'ocorrencia' &&
    ['enviado', 'reprovado'].includes(doc.status) && (
      <button
        onClick={aprovar}
        className="flex items-center gap-1 bg-green-100 hover:bg-green-200 text-green-800 border border-green-300 px-3 py-1.5 rounded-md shadow-sm transition"
      >
        <FaPaperPlane size={14} />
        Aprovar
      </button>
  ...
  {userData.setor === 'ocorrencia' &&
    ...
    doc.status !== 'saldo_liberado' && (
      <button
        ...
        className="flex items-center gap-1 bg-red-100 hover:bg-red-200 text-red-800 border border-red-300 px-3 py-1.5 rounded-md shadow-sm transition"
      >
        <FaPaperclip size={14} />
        Reprovar
      </button>
